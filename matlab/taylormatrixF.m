function N=taylormatrixF(S,pKa,pH,T,I_m,constant)
...
I=sym('I',[l 1]);
I_m(isnan(I_m)) = 0.25 ; 
for i=1:length(index_measured)
        k=index_measured(i);
        diss = pKa(k);
        T=T(i);
        pH=pH(i);
        dG0s = cumsum(-[0, pka(k,1:no_pseudo(k))] * R * T * log(10));
        ...
        f=-R*T*log(sum(exp(dG0_prime_vector / (-R * T))));
        
        if ~isequal(f,sym('0'))
            g=subs(f,pka,pka_m);
            N(index_measured(i),1)=vpa(subs(g,I,I_m));
            F=symvar(f);
            h=jacobian(f,F);
              if ~constant %For linear data reconciliation, only constant terms are required
               if ~isempty(F)
                 ...
                 N(index_measured(i),1+t+n+(i))=diffs(1);
                     for k=2:length(F)
                         str=char(F(k));
                         ...
                         N(index_measured(i),L+index_pseudo)=diffs(k);
                      end
              end
            end 
       end
